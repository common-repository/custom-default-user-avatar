...
class Custom_Default_User_Avatar_Update {
	/**
	 * Constructor
	 * @since 1.8
	...
	public function wpua_default_avatar() {
		...
		if ( $avatar_default == $mustache_original ) {
			update_option( 'avatar_default', 'custom_default_user_avatar' );
		}
		...
		update_option( 'custom_default_user_avatar_default_avatar_updated', '1' );
	}
	...
	public function wpua_user_meta() {
		...
		if ( 'custom_default_user_avatar' != $wpua_metakey ) {
			$users = get_users();

			// Move current user metakeys to new metakeys
			foreach ( $users as $user ) {
				$wpua = get_user_meta( $user->ID, 'custom_default_user_avatar', true );

				if ( ! empty( $wpua ) ) {
					update_user_meta( $user->ID, $wpua_metakey, $wpua );
					delete_user_meta( $user->ID, 'custom_default_user_avatar' );
				}
			}
		}

		update_option( 'custom_default_user_avatar_users_updated', '1' );
	}
	...
	public function wpua_media_state() {
		...
		foreach( $wpuas as $usermeta ) {
			add_post_meta( $usermeta->meta_value, '_wp_attachment_custom_default_user_avatar', $usermeta->user_id );
		}

		update_option( 'custom_default_user_avatar_media_updated', '1' );
	}
}
...
function wpua_update_init() {
	...
	if ( ! isset( $wpua_update ) ) {
		$wpua_update = new Custom_Default_User_Avatar_Update();
	}
