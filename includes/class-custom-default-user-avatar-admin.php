...
class Custom_Default_User_Avatar_Admin {
	/**
	 * Constructor
	 * @since 1.8
	...
	public function __construct() {
		...
		register_activation_hook( WPUA_DIR . 'custom-default-user-avatar.php', array( $this, 'wpua_options' ) );
		...
		add_action( 'load-avatars_page_custom-default-user-avatar-library', array( $this, 'wpua_bulk_actions' ) );

		// Translations
		load_plugin_textdomain( 'custom-user-avatar', '', WPUA_FOLDER . '/languages' );
	...
	public function wpua_options() {
		add_option( 'avatar_default_custom_default_user_avatar',       '' );
	...
	public function wpua_enqueue_scripts( $hook_suffix ) {
		if ( 'avatars_page_custom-default-user-avatar-library' != $hook_suffix ) {
			return;
		}
	...
	public function wpua_bulk_actions() {
		...
		if ( $doaction ) {
			...
			$location = esc_url( add_query_arg( array( 'page' => 'custom-default-user-avatar-library' ), 'admin.php' ) );
				...
				case 'delete':
					...
					foreach ( $post_ids as $post_id_del ) {
						if ( ! current_user_can( 'delete_post', $post_id_del ) ) {
							wp_die( __('You are not allowed to delete this post.', 'custom-user-avatar' ) );
						}

						if ( ! wp_delete_attachment( $post_id_del ) ) {
							wp_die( __( 'Error in deleting.','custom-user-avatar' ) );
						}
					}
	...
	public function wpua_admin() {
		add_menu_page(
            __( 'WP User Avatar 2.0', 'custom-user-avatar' ),
            __( 'WP User Avatar 2.0', 'custom-user-avatar' ),
            'manage_options',
            'custom-user-avatar',
		...
		add_submenu_page(
            'custom-user-avatar',
            __( 'Settings', 'custom-user-avatar' ),
            __( 'Settings' , 'custom-user-avatar' ),
            'manage_options',
            'custom-user-avatar',
            array( $this, 'wpua_options_page' )
		...
		$hook = add_submenu_page(
            'custom-user-avatar',
            __( 'Library','custom-user-avatar' ),
            __( 'Library', 'custom-user-avatar' ),
            'manage_options',
            'custom-default-user-avatar-library',
	...
	public function wpua_is_menu_page() {
		...
		$is_menu_page = ( 'admin.php' == $pagenow && isset( $page ) && 'custom-user-avatar' == $page ) ? true : false;
	...
	public function wpua_media_screen_option() {
		add_screen_option( 'per_page', array(
			'label'   => __( 'Avatars', 'custom-user-avatar' ),
			'default' => 10,
	...
	public function wpua_register_settings() {
		...
		$settings[] = register_setting( 'wpua-settings-group', 'avatar_default_custom_default_user_avatar'                );
	...
	public function wpua_add_default_avatar() {
		...
		$avatar_defaults = array(
			'mystery'          => __( 'Mystery Man',           'custom-user-avatar'),
			'blank'            => __( 'Blank',                 'custom-user-avatar'),
			'gravatar_default' => __( 'Gravatar Logo',         'custom-user-avatar'),
			'identicon'        => __( 'Identicon (Generated)', 'custom-user-avatar'),
			'wavatar'          => __( 'Wavatar (Generated)',   'custom-user-avatar'),
			'monsterid'        => __( 'MonsterID (Generated)', 'custom-user-avatar'),
			'retro'            => __( 'Retro (Generated)',     'custom-user-avatar')
		);
		...
		// Default Avatar is custom_default_user_avatar, check the radio button next to it
		$selected_avatar = ( 1 == (bool) $wpua_disable_gravatar || 'custom_default_user_avatar' == $avatar_default ) ? ' checked="checked" ' : '';
		...
		// Add WPUA to list
		$wpua_list  = sprintf(
			'<label><input type="radio" name="avatar_default" id="custom_default_user_avatar_radio" value="custom_default_user_avatar" %s /> ',
			$selected_avatar
		);

		$wpua_list .= preg_replace( "/src='(.+?)'/", "src='\$1'", $avatar_thumb_img );
		$wpua_list .= ' ' . __( 'WP User Avatar 2.0', 'custom-user-avatar' ) . '</label>';
		$wpua_list .= '<p id="wpua-edit"><button type="button" class="button" id="wpua-add" name="wpua-add" data-avatar_default="true" data-title="' . __('Choose Image') . ': ' . __('Default Avatar') . '">' . __( 'Choose Image','custom-user-avatar' ) . '</button>';
		$wpua_list .= '<span id="wpua-remove-button"' . $hide_remove . '><a href="#" id="wpua-remove">' . __('Remove','custom-user-avatar') . '</a></span><span id="wpua-undo-button"><a href="#" id="wpua-undo">' . __('Undo','custom-user-avatar') . '</a></span></p>';
		$wpua_list .= '<input type="hidden" id="custom-default-user-avatar" name="avatar_default_custom_default_user_avatar" value="' . $wpua_avatar_default . '">';
	...
	public function wpua_whitelist_options( $options ) {
		$options['discussion'][] = 'avatar_default_custom_default_user_avatar';
	...
	public function wpua_action_links( $links, $file ) {
		if( basename( dirname( $file ) ) == 'custom-user-avatar' ) {
			$links[] = sprintf(
				'<a href="%s">%s</a>',
				esc_url( add_query_arg( array( 'page' => 'custom-user-avatar' ), admin_url( 'admin.php' ) ) ),
				__( 'Settings', 'custom-user-avatar' )
			);
		}
	...
	public function wpua_row_meta( $links, $file ) {
		if ( 'custom-user-avatar' == basename( dirname( $file ) ) ) {
			$links[] = sprintf(
				'<a href="%s" target="_blank">%s</a>',
				esc_url( __( 'https://wordpress.org/support/plugin/custom-default-user-avatar/', 'custom-user-avatar' ) ),
				__( 'Support Forums', 'custom-user-avatar' )
			);
		}
	...
	public function wpua_add_column($columns) {
		return $columns + array( 'custom-user-avatar' => __( 'Profile Picture', 'custom-user-avatar' ) );
	}
	...
	public function wpua_show_column( $value, $column_name, $user_id ) {
		...
		if ( 'custom-user-avatar' == $column_name ) {
			$value = $wpua_image;
		}
	...
	public function _wpua_get_list_table( $class, $args = array() ) {
		require_once( WPUA_INC . 'class-custom-default-user-avatar-list-table.php' );

		$args['screen'] = 'custom-user-avatar';
	...
	public function wpua_add_media_state($states) {
		...
		$is_wpua = isset( $post->ID ) ? get_post_custom_values( '_wp_attachment_custom_default_user_avatar', $post->ID ) : '';

		if ( ! empty( $is_wpua ) ) {
			$states[] = __( 'Profile Picture','custom-user-avatar' );
		}

		if ( ! empty ( $wpua_avatar_default ) && isset( $post->ID ) && ( $wpua_avatar_default == $post->ID ) ) {
			$states[] = __( 'Default Avatar', 'custom-user-avatar' );
		}
...
function wpua_admin_init() {
	...
	if ( ! isset( $wpua_admin ) ) {
		$wpua_admin = new Custom_Default_User_Avatar_Admin();
	}
