...
class Custom_Default_User_Avatar_List_Table extends WP_List_Table {
	/**
	 * Constructor
	 * @since 1.8
	 * @param array $args
	...
	public function search_box( $text, $input_id ) {
		...
		<p class="search-box">
			...
			<input type="hidden" id="page" name="page" value="custom-default-user-avatar-library" />
	...
	public function get_views() {
		...
		$_total_posts = count( array_filter( $avatars ) );
                $post_mime_type = filter_input(INPUT_GET, 'post_mime_type');
                $status = filter_input(INPUT_GET, 'status');
		$class        = ( empty($post_mime_type) && ! isset( $status ) ) ? ' class="current"' : '';

		$type_links['all']  = sprintf(
			' <a href="%s">',
			esc_url( add_query_arg( array(
				'page' => 'custom-default-user-avatar-library',
			), 'admin.php') )
		);
		$type_links['all'] .= sprintf(
			/* translators: uploaded files */
			_x( 'All %s', 'uploaded files', 'custom-user-avatar' ),
	...
	public function get_bulk_actions() {
		...
		$actions['delete'] = __( 'Delete Permanently','custom-user-avatar' );
	...
	public function no_items() {
		_e( 'No media attachments found.', 'custom-user-avatar' );
	}
	...
	public function get_columns() {
		...
		$columns['icon']   = '';
		$columns['title']  = _x( 'File', 'column name', 'custom-user-avatar' );
		$columns['author'] = __( 'Author','custom-user-avatar', 'custom-user-avatar' );
		$columns['parent'] = _x( 'Uploaded to', 'column name', 'custom-user-avatar' );
		$columns['date']   = _x( 'Date', 'column name', 'custom-user-avatar' );
					...
					switch ( $column_name ) {
							...
							<th scope="row" class="check-column">
								<?php if ( $user_can_edit ) : ?>
									<label class="screen-reader-text" for="cb-select-<?php the_ID(); ?>">
										...
										printf( __( 'Select %s','custom-user-avatar' ), $att_title );
										?>
									</label>
							...
							?>
								...
								if ( $thumb = $wpua_functions->wpua_get_attachment_image( $post->ID, array( 80, 60 ), true ) ) {
									if ( $this->is_trash || ! $user_can_edit ) {
										...
										?>
										<a href="<?php echo get_edit_post_link( $post->ID, true ); ?>" title="<?php echo sprintf( __( 'Edit %s' ), sprintf( '&#8220;%s&#8221;', $att_title ) ); ?>">
											<?php echo $thumb; ?>
										</a>
										<?php
									}
								...
								<strong>
									...
									if ( $this->is_trash || ! $user_can_edit ) {
										...
										<a
											...
											title="<?php echo sprintf( __( 'Edit %s' ), sprintf( '&#8220;%s&#8221;', $att_title ) ); ?>"
										>
						...
						case 'date':
							if( '0000-00-00 00:00:00' == $post->post_date ) {
								$h_time = __( 'Unpublished','custom-user-avatar' );
							} else {
								...
								if ( ( abs( $t_diff = time() - $time ) ) < DAY_IN_SECONDS ) {
									if ( 0 > $t_diff ) {
										$h_time = sprintf(
											...
											_x( '%s from now', 'time from now', 'custom-user-avatar' ),
											human_time_diff( $time)
										);
									} else {
										$h_time = sprintf(
											/* translators: time ago */
											_x( '%s ago', 'time ago', 'custom-user-avatar' ),
											human_time_diff( $time )
										);
									}
								} else {
									$h_time = mysql2date( __( 'Y/m/d', 'custom-user-avatar' ), $m_time );
								}
							}
							?>

                                                        <td <?php echo esc_attr($attributes); ?>><?php echo esc_html($h_time); ?></td>
		...
		if ( current_user_can( 'delete_post', $post->ID ) ) {
			...
			if ( $this->is_trash || ! EMPTY_TRASH_DAYS || ! MEDIA_TRASH ) {
				...
				$actions['delete'] = sprintf(
					...
					__( 'Delete Permanently', 'custom-user-avatar' )
				);
			}
		}
